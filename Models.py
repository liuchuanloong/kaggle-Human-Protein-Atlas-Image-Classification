...
class SFPAModule(nn.Module):

    def __init__(self, in_ch, out_ch):
        super(SFPAModule, self).__init__()
    ...
    def forward(self, x):
        ...
        x3 = nn.Upsample(size=(h // 2, w // 2), mode='bilinear', align_corners=True)(x3)
        x2 = self.conv2(x2)
        x2 = nn.Upsample(size=(h // 2, w // 2), mode='bilinear', align_corners=True)(x2)
        x1 = self.conv1(x1)

        m_batchsize, C, height, width = x1.size()
        proj_query = x3.view(m_batchsize, -1, width*height).permute(0, 2, 1)
        proj_key = x2.view(m_batchsize, -1, width*height)
        energy = torch.bmm(proj_query, proj_key)
        attention = self.softmax(energy)
        proj_value = x1.view(m_batchsize, -1, width*height)

        out = torch.bmm(proj_value, attention.permute(0, 2, 1))
        out = out.view(m_batchsize, C, height, width)

        out = self.gamma*out + x1
        out = nn.Upsample(size=(h, w), mode='bilinear', align_corners=True)(out)
