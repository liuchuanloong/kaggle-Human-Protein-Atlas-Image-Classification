...
class model34_DeepSupervion(nn.Module):
    ...
    def forward(self, x):
        ...
        center_64_flatten = center_64.view(center_64.size(0), -1)#(None,64)
        center_fc = self.center_fc(center_64_flatten)#(None,2)
        ...
        x_no_empty = self.logits_no_empty(hypercol)#(None,1,128,128)
        hypercol_add_center = torch.cat((
            hypercol,
            F.upsample(center_64, scale_factor=128,mode='bilinear')),1)#(None,64+320,128,128)

        x_final = self.logits_final( hypercol_add_center)#(None,1,128,128)
        return center_fc, x_no_empty, x_final


class model34B_DeepSupervion(nn.Module):
    def __init__(self, num_classes=1, mask_class = 2):
        super(model34_DeepSupervion, self).__init__()
        ...
        self.conv4 = self.encoder.layer3
        self.conv5 = self.encoder.layer4
        self.scse = SCSEBlock(512)
        self.center_global_pool = nn.AdaptiveAvgPool2d([1,1])
        self.center_conv1x1 = nn.Conv2d(512, 64, kernel_size=1)
        self.center_fc = nn.Linear(64, mask_class)

        self.up_road = nn.Sequential(nn.AdaptiveAvgPool2d([1,1]),
                                     nn.Conv2d(512,256,kernel_size=1,padding=0,stride=1),
                                     nn.BatchNorm2d(256),
                                     nn.ReLU(inplace=True),
                                     )
        self.mid_road = nn.Sequential(nn.Conv2d(512,256,kernel_size=1,padding=0,stride=1),
                                      nn.BatchNorm2d(256),
                                      nn.ReLU(inplace=True))

        self.avg = nn.AvgPool2d(2,stride=2)

        self.conv5x5_1 = nn.Conv2d(512, 512, kernel_size=3, padding=1, stride=1)
        self.conv3x3_1 = nn.Conv2d(512, 512, kernel_size=5, padding=2, stride=1)

        self.conv5x5_2 = nn.Conv2d(512,256,kernel_size=3,padding=1,stride=1)
        self.conv3x3_2 = nn.Conv2d(512,256,kernel_size=5,padding=2,stride=1)
    ...
    def forward(self, x):
        ...
        x1 = self.up_road(scse)
        x1 =  F.upsample(x1, scale_factor=8,mode='bilinear')

        x2 = self.mid_road(scse)

        x3 = self.avg(scse)
        x3 = self.conv5x5_1(x3)
        x5 = self.conv5x5_2(x3)

        x4 = self.avg(x3)
        x4 = self.conv3x3_1(x4)
        x4 = self.conv3x3_2(x4)
        x4 =  F.upsample(x4, scale_factor=2,mode='bilinear')
        x5 += x4
        x5 = F.upsample(x5, scale_factor=2,mode='bilinear')
...
class model50B_DeepSupervion(nn.Module):
    def __init__(self, num_classes=1):
        super(model50B_DeepSupervion, self).__init__()

        self.num_classes = num_classes
        baseWidth = 4
        cardinality = 32
        self.encoder = resnext50_ibn_a(baseWidth, cardinality)
        self.relu = nn.ReLU(inplace=True)
        self.conv1 = nn.Sequential(self.encoder.conv1,
                                   self.encoder.bn1,
                                   self.encoder.relu)
...
class model50A_DeepSupervion(nn.Module):
    def __init__(self, num_classes=28):
        ...
        self.conv1x1 = nn.Conv2d(4,3,kernel_size=1,padding=0,stride=1)
    ...
    def forward(self, x):
        ...
        conv4 = self.conv4(conv3) #1/16
        conv5 = self.conv5(conv4) #1/32
        conv6 = self.center(conv5)
        pool = self.avg_pool(conv6)
